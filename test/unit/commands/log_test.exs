defmodule Ontogen.Commands.LogTest do
  ...
  describe "Repo.dataset_log/1" do
    test "on a clean repo without commits" do
      assert Repo.dataset_log() == {:error, :no_head}
    end
  ...
  describe "Repo.resource_log/1" do
    test "on a clean repo without commits" do
      assert Repo.resource_log(EX.S1) == {:error, :no_head}
    end

    test "full native history" do
      history = init_resource_history()

      assert Repo.resource_log(EX.S1) == {:ok, history}
    end

    test "full native history up to certain commit" do
      [fourth, third, second, first] = history = init_resource_history()

      assert Repo.resource_log(EX.S1, to_commit: first.__id__) == {:ok, Enum.slice(history, 0..2)}

      assert Repo.resource_log(EX.S1, to_commit: second.__id__) ==
               {:ok, Enum.slice(history, 0..1)}

      assert Repo.resource_log(EX.S1, to_commit: third.__id__) == {:ok, Enum.slice(history, 0..0)}
      assert Repo.resource_log(EX.S1, to_commit: fourth.__id__) == {:ok, []}
    end

    test "full native history from a certain commit" do
      [fourth, third, second, first] = history = init_resource_history()

      assert Repo.resource_log(EX.S1, from_commit: fourth.__id__) == {:ok, history}

      assert Repo.resource_log(EX.S1, from_commit: third.__id__) ==
               {:ok, Enum.slice(history, 1..3)}

      assert Repo.resource_log(EX.S1, from_commit: second.__id__) ==
               {:ok, Enum.slice(history, 2..3)}

      assert Repo.resource_log(EX.S1, from_commit: first.__id__) ==
    ...
    test "full native history from a certain commit to a certain commit" do
      ...
      assert Repo.resource_log(EX.S1, from_commit: fourth.__id__, to_commit: first.__id__) ==
               {:ok, Enum.slice(history, 0..2)}

      assert Repo.resource_log(EX.S1, from_commit: third.__id__, to_commit: second.__id__) ==
               {:ok, Enum.slice(history, 1..1)}
    end
    ...
    test "when from_commit comes later than to_commit" do
      ...
      assert Repo.resource_log(EX.S1, from_commit: second.__id__, to_commit: third.__id__) ==
               {:ok, []}

      assert Repo.resource_log(EX.S1, from_commit: first.__id__, to_commit: fourth.__id__) ==
               {:ok, []}
    end
    ...
    test "full raw history" do
      ...
      assert Repo.resource_log(EX.S1, type: :raw) ==
               {:ok,
                RDF.graph(Enum.map(history, &Grax.to_rdf!(&1)))
                |> Graph.add_prefixes(RDF.standard_prefixes())
                |> Graph.add_prefixes(og: Og, rtc: RTC)}
